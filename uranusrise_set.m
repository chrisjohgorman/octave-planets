function [uranusrise, uranusset] = uranusrise_set (year, month, day, hour, latitude, longitude)
    h = -0.833;
    d = day_number(year, month, day, hour);
    [x1, y1, z1, oblecl, L] = sun_rectangular(d);
    [RA, Decl, r, az, alt] = uranus(d, latitude, longitude, hour);
    GMST0 = (L + 180) / 15;
    ...
    uranusrise = datestr(jr/24, 'HH:MM');
    uranusset = datestr(js/24, 'HH:MM');
end
