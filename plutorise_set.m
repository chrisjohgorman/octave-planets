function [plutorise, plutoset] = plutorise_set (year, month, day, hour, latitude, longitude)
    ...
    [RA, Decl, r, az, alt] = pluto(d, latitude, longitude, hour);
    GMST0 = (L + 180) / 15;
    ...
    if (cos_lha > 1)
        error("Pluto is always below our altitude limit.");
    elseif (cos_lha < -1)
        error("Pluto is always above our altitude limit.");
    end
    LHA = acosd(cos_lha)/15.04107;
    time = localtime(time);
    jr = UT_Planet_in_south - LHA + time.gmtoff/3600;
    js = UT_Planet_in_south + LHA + time.gmtoff/3600;
    plutorise = datestr(jr/24, 'HH:MM');
    plutoset = datestr(js/24, 'HH:MM');
end
